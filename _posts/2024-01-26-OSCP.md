...
-------------------------------------------------------------------------------
 I honestly enjoyed studying for the OSCP because I felt that I'm actually doing something useful, I also engaged with the community unlike school where I was wasting time studying just to pass a stupid exam. I did approximately 7 hours per day reading the course and doing the exercises. I finished the course material in nearly 2 months. I also did the labs, and ofcourse I was stuck on some machines but I got hints from members and staff in offsec's discord server.
...
There is 6 machines in the exam: 3 standalone machines (independent challenges) and 1 AD Set (3 machines in the Set). You need 70 points to pass, each standalone machine grants you 20 points (10 points for local.txt and 10 points for proof.txt). For the AD set you need to pwn all machines including DC to get 40 points. So for example if you compromise the first and second machine of the AD set without the third (DC), you will get no points.

You can earn +10 bonus points from completing 80% course material and exercises of each module + rooting +30/57 Labs.

I started by doing HackTheBox TJ Null's list boxes, Then OSCP's LAB machines (OSCP A,B,C, MedTech, Relia) and finally Proving grounds TJ Null's list. Every machine that I had finished was noted and saved in my documents for later use. I also did skylark from the offsec's labs but that one is optional.
...
Next, I moved on to the standalone machines. I couldn't find an attack vector on the first one so I immediately moved to the next. By 15:00 AM I had both local and proof flags. By now I new I had enough points to pass but I had to check the last box. I eventually found an attack vector on the last machine and got local and proof flags by 17:33 AM.

Keep in mind that I have faced so many rabbit holes, and took too many breaks when I was stuck on initial foothold or privilege escalation.
...
>Useful websites : [https://gtfobins.github.io/](https://gtfobins.github.io/)
| [https://wadcoms.github.io/](https://wadcoms.github.io/) | [https://www.revshells.com/](https://www.revshells.com/) | 
