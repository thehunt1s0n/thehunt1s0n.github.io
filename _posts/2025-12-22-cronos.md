...
Let's capture the request using burpsuite.
...
This file seems to be type php
So let's try upload a php reverse shell from my attacker's machine and change the name to artisan then execute it as root using sudo.</span>
...
Don't forget to change the attacker's ip and the port
